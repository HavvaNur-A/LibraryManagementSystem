...
public class MainFrame extends JFrame {
    ...
    public MainFrame() {
        try {
            ...
            initializeUI();
            setupWindowCloseListener(); // Pencere kapanma işlemini ekle
        } catch (Exception e) {
    ...
    // The process of saving data when the window closes
    ...
    private void addBook() {
        ...
        if (controller.addBook(new Book(id, title, author))) {
            showSuccess("Book added successfully!");
            clearBookFields();
            listBooks();
        } else {
            showError("Book could not be added. ID may already exist!");
        }
    }
    ...
    private void performBookSearch() {
        ...
        if (exactBook != null) {
            String status = exactBook.isAvailable() ? "AVAILABLE" : "BORROWED";
            ...
            showInfo("Status Of Book", message);
        } else if (results.size() == 1) {
            Book singleBook = results.get(0);
            String status = singleBook.isAvailable() ? "AVAILABLE" : "BORROWED";
            String message = String.format(
            	"Book ID: %s\nTitle: %s\nAuthor: %s\nStatus: %s",
                singleBook.getId(),
                singleBook.getTitle(),
            ...
            showInfo("Search Result", results.size() + "The Book Was Found.");
        }
    }
    ...
    private void addMember() {
        ...
        if (controller.addMember(new Member(id, name, email))) {
            showSuccess("Member added successfully!");
            clearMemberFields();
            listMembers();
        } else {
            showError("Member could not be added. ID may already exist!");
        }
    }
    ...
    private void performMemberSearch() {
        ...
        if (query.isEmpty()) {
            showError("Please enter your member ID, name or e-mail to the search!");
            return;
        }
        ...
        if (results.isEmpty()) {
            showInfo("Result of Search", "No member was found who met the required criteria: " + query);
        ...
        if (exactMember != null) {
            ...
            showInfo("Member Information", message);
        } else if (results.size() == 1) {
            ...
            showInfo("Result of Search", results.size() + "Member Found.");
        }
    }
    ...
    private void borrowBook() {
        ...
        // Member Control
        Member member = controller.getMemberById(memberId);
        if (member == null) {
            showError("Member ID does not exist!");
            return;
        }
